    ...
    def _adc_read_channel_single_ended(self,adc=None,adc_ch=None):
        if adc is not None and adc_ch is not None:
            ...
            if adc == 'ADC_0' or adc == 0:
                ...
                return self._adc_1.read_channel_single_ended(adc_ch, internal_ref_on=True, ad_on=True)
            else:
...
if __name__ == "__main__":
    try:
        eddy = EddyEps(rev='A', smbus_num=1)

        # Read all measurements with small delays
        dataDict = dict(
            unix_time = time.time(),
            batt_voltage = eddy.get_voltage_vbatt_raw(),
            batt_current = eddy.get_current_vbatt_raw(),
        )
        #time.sleep(0.05)
        dataDict.update(dict(
            v_3v3 = eddy.get_voltage_3v3(),
            i_3v3 = eddy.get_current_3v3(),
        ))
        #time.sleep(0.05)
        dataDict.update(dict(
            v_5v0 = eddy.get_voltage_5v0(),
            i_5v0 = eddy.get_current_5v0(),
        ))
        #time.sleep(0.05)
        dataDict.update(dict(
            v_vbatt = eddy.get_voltage_vbatt(),
            i_vbatt = eddy.get_current_vbatt(),
        ))
        #time.sleep(0.05)
        dataDict.update(dict(
            reg_temp_3v3 = eddy.get_temp_3v3_reg(),
            reg_temp_5v0 = eddy.get_temp_5v0_reg()
        ))
        # eddy.print_measurement_table() # for testing
        # dataDict = dict(
        #     batt_voltage = eddy.get_voltage_vbatt_raw(),
        #     batt_current = eddy.get_current_vbatt_raw(),
        #     v_3v3 = eddy.get_voltage_3v3(),
        ...
        # print("I_3v3:", eddy.get_current_3v3())
        # print("I_VBATT:", eddy.get_current_vbatt())
        # print("V_VBATT:", eddy.get_voltage_vbatt())
